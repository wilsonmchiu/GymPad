    ...
    public void updateData(){
        btnUpdateData.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (temp > 0) {
                    boolean update = MONWORKOUT.updateMonData(editID.getText().toString(), editName.getText().toString(),
                            editSets.getText().toString(), editReps.getText().toString(), editWeight.getText().toString());
                    if (update == true) {
                        Toast.makeText(MondayAdd.this, "Successfully Updated Data!", Toast.LENGTH_LONG).show();
                    } else {
                        Toast.makeText(MondayAdd.this, "Something Went Wrong :(.", Toast.LENGTH_LONG).show();
                    }
                } else {
                    Toast.makeText(MondayAdd.this, "You Must Enter An ID to Update :(.", Toast.LENGTH_LONG).show();
                }
            }
        });
    }
    public void deleteData(){
        btnDeleteData.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                int temp = editID.getText().toString().length();
                if(temp > 0){
                    Integer deleteRow = MONWORKOUT.deleteMonData(editID.getText().toString());
                    if(deleteRow > 0){
                        Toast.makeText(MondayAdd.this, "Successfully Deleted The Data!", Toast.LENGTH_LONG).show();
                    }else{
                        Toast.makeText(MondayAdd.this, "Something went wrong :(.", Toast.LENGTH_LONG).show();
                    }
                }else{
                    Toast.makeText(MondayAdd.this, "You Must Enter An ID to Delete :(.", Toast.LENGTH_LONG).show();
                }
            }
        });
    }
